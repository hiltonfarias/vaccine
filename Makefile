...
build:
	mvn clean install; \
	docker build --force-rm -t vaccine-k8s .
...
run-app: stop-app rm-app
	docker run --name vaccine -p 8080:8080 -d -e DATABASE_SERVER_NAME=mysql57 --link mysql57:mysql57 java-k8s:latest
...
k-setup:
	...
	kubectl create namespace dev-vaccine
...
k-build-image:
	eval $$(minikube -p dev.to docker-env) && docker build --force-rm -t vaccine-k8s .;

k-cache-image:
	minikube cache add vaccine-k8s;

k-deploy-app:
	kubectl apply -f k8s/vaccine/;

k-delete-app:
	kubectl delete -f k8s/vaccine/;
